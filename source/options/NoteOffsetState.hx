...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var stageData:StageFile = StageData.getStageFile('tsftw');

		var sky:FlxSprite = new FlxSprite(-430, -176);
		sky.scrollFactor.set(0.35, 0.35);
		sky.loadGraphic(Paths.image('sky', 'week1'));
		sky.setGraphicSize(Std.int(sky.width * 1.5));
		sky.updateHitbox();
		add(sky);

		var clouds:BGSprite = new BGSprite('clouds', -490, -209, 0.45, 0.45);
		clouds.setGraphicSize(Std.int(clouds.width * 1.5));
		clouds.loadGraphic(Paths.image('clouds', 'week1'));
		clouds.updateHitbox();
		add(clouds);

		var elim:BGSprite = new BGSprite('elim', 920, -30, 0.47,0.47);
		elim.loadGraphic(Paths.image('elim', 'week1'));
		elim.updateHitbox();
		add(elim);

		var purpHill:BGSprite = new BGSprite('hills purp', 175, 100 -4.3, 0.55, 0.55);
		purpHill.setGraphicSize(Std.int(purpHill.width * 1.5));
		purpHill.loadGraphic(Paths.image('hills purp', 'week1'));
		purpHill.updateHitbox();
		add(purpHill);
		
		var hills:BGSprite = new BGSprite('hills', -428, -93, 0.6, 0.6);
		hills.loadGraphic(Paths.image('hills', 'week1'));
		hills.setGraphicSize(Std.int(hills.width * 1.5));
		hills.updateHitbox();
		add(hills);

		var foreground:BGSprite = new BGSprite('foreground', -430, 200, 0.8, 0.8);
		foreground.loadGraphic(Paths.image('foreground', 'week1'));
		foreground.setGraphicSize(Std.int(foreground.width * 1.5));
		foreground.updateHitbox();
		add(foreground);

		var ship:BGSprite = new BGSprite('ship', 373, 322, 0.8, 0.8, ['ship idle']);
		ship.loadGraphic(Paths.image('ship', 'week1'));
		ship.animation.addByPrefix('fly', 'ship fly', 24, false);
		add(ship);

		var ground:BGSprite = new BGSprite('ground', -697, 249, 0.95, 0.95);
		ground.loadGraphic(Paths.image('ground', 'week1'));
		ground.setGraphicSize(Std.int(ground.width * 1.5));
		ground.updateHitbox();
		add(ground);

		// Characters
		gf = new Character(336, 150, 'gfObjectBig');
		gf.x += gf.positionArray[0];
		gf.y += gf.positionArray[1];
		gf.scrollFactor.set(0.95, 0.95);
		boyfriend = new Character(845, 250, 'bf-object', true);
	...
	override public function update(elapsed:Float)
	{
		...
		if(controls.BACK)
		{
			...
			FlxG.sound.playMusic(Paths.music('StrugglinOnAFridayNight'), 1, true);
	...
	override public function beatHit()
	{
		...
		if(curBeat % 4 == 2)
		{
			...
			if(zoomTween != null) zoomTween.cancel();
			zoomTween = FlxTween.tween(FlxG.camera, {zoom: 0.9}, 1, {ease: FlxEase.circOut, onComplete: function(twn:FlxTween)
				{
					zoomTween = null;
				}
			});
